  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: specData } = await supabase
        .from('specialists').select('*').eq('id', authUser.id).single()
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        {specialist?.kyc_status === 'rejected' && (
          <motion.div variants={fadeUp} custom={0} initial="hidden" animate="show" className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-red-600 mt-0.5">{isAr ? 'يرجى إعادة تقديم المستندات' : 'Please resubmit your documents'}</p>
            </div>
            <Link href={`/${safeLang}/onboarding`} className="bg-red-500 hover:bg-red-600 text-white text-xs font-semibold px-4 py-2 rounded-xl transition-all">
        ...
        <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-4" variants={fadeUp} custom={1} initial="hidden" animate="show">
          {[
            ...
            { icon: IconUsers,    value: `${completedCount}`,          label: isAr ? 'مكتملة' : 'Completed',        color: 'bg-white border border-gray-100' },
