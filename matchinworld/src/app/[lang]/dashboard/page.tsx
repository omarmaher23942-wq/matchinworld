...
import {
  IconCalendar, IconSearch, IconClock,
  IconUser, IconLogout, IconChevronRight,
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-4" variants={fadeUp} custom={1} initial="hidden" animate="show">
          {[
            ...
            <Link key={i} href={action.href} className={`flex flex-col items-center gap-2 p-4 rounded-2xl font-medium text-sm transition-all hover:-translate-y-1 hover:shadow-md ${action.color}`}>
              <action.icon size={22} />
        ...
        {/* Wallet */}
        {wallet && (
          <motion.div variants={fadeUp} custom={2} initial="hidden" animate="show" className="bg-gradient-to-r from-blue-600 to-violet-600 rounded-3xl p-6 text-white">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
              ...
              {pendingBookings.map(b => (
                <div key={b.id} className="flex items-center justify-between bg-white rounded-xl p-3 mt-2">
                  ...
                  <Link href={`/${safeLang}/pay/${b.id}`} className="bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold px-4 py-2 rounded-xl transition-all">
        ...
        <motion.div variants={fadeUp} custom={4} initial="hidden" animate="show">
          ...
          {upcomingBookings.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
              ...
              <Link href={`/${safeLang}/match`} className="inline-flex items-center gap-2 bg-blue-600 text-white text-sm font-semibold px-5 py-2.5 rounded-xl hover:bg-blue-700 transition-all">
